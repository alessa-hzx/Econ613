...
library(knitr)
kable(school_data[1:20, ], caption = "School level data")
```
## EX3
```{r}
...
```{r echo = FALSE, results='asis'}
library(knitr)
kable(school_dis[1:20, ], caption = "Schools distances")
```
## EX4
```{r}
# rank 1 choice
...
```{r echo = FALSE, results='asis'}
library(knitr)
kable(describe_rank, caption = "Descriptive characteristics1")
```
```{r}
...
```{r echo = FALSE, results='asis'}
library(knitr)
kable(des_quantile, caption = "Descriptive characteristics2")
```
## EX5-6
Y~X1
```{r}
...
beta1
```
We can see that the correlation coefficient between Y and X1 is 1.2778, which is
slightly larger than 1.2, which is the true coefficient. This may due to the number of 
...
```
## EX7
Linear probability model: $P[y_i = 1|x_i] = x_i^T \beta$
```{r}
result1 = solve(t(X) %*% X) %*% t(X) %*% ydum
result1
```
These parameters means that, ceteris paribus, for every 1 unit increase of X1, the probability of
Ydum = 1 will increase by 0.136; for every 1 unit increase of X2, the probability 
of Ydum = 1 will decrease by 0.1; for every 1 unit increase of X3, the probability
of Ydum = 1 will increase by 0.02.
```{r}
p = 3
hat_ydum1 = X %*% result1
...
```
The F-test shows that the parameters are significant at 0.01 level.

Probit model: $P[y_i = 1|x_i] = \phi(x_i^T\beta)$
```{r}
...
result2$par
```
These parameters indicates that, ceteris paribus, an increase of X1 or X3 will 
increases the probability of ydum = 1, whereas an increase of X2 will decreases
the probability of ydum = 1.
```{r}
p = 3
...
Logit model: $P[y_i = 1|x_i] = \frac{1}{1+e^{-x_i^T\beta}}$
```{r}
...
increases the probability of ydum = 1, whereas an increase of X2 will decreases
the probability of ydum = 1. 
Noted that parameters for the logit model are generally larger in magnitude 
than those for the probit model, as well as those for the linear model.
```{r}
p = 3
